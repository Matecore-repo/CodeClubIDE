export function systemPrompt(
  ...
  prompt += `\n\nTOOL SELECTION: Use read, write, edit, and delete for files. Prefer subtool=rust (AST mode) for programming work: it exposes workspace/folder/file/section/symbol nodes, returns metadata without unnecessary content, supports precise node or line-range operations, and requires baseHash for safe mutations. Use search/topographic (AST search) to find nodes by name, type, language, or path before reading the tree; use search/classic only when searching actual file text or regex. In AST mode, inspect graph/architecture schema.astNodeTypes before assuming node types: depending on the language/parser, methods may appear as function instead of method (notably C/C++). If nodeType=method returns nothing, retry nodeType=function before falling back to text. To read a specific AST node, pass both filePath and nodeId; nodeId alone is not enough. Then read only the target node or range. Use write/rust operation=create-file with content to create a populated file; use create-folder only for directories and insert to add a new section/node to an existing file. AST mode can also replace or delete an entire file: omit nodeId/startLine/endLine and provide the file baseHash. Use file subtool=classic only for small whole-file operations or when structural parsing cannot identify a useful node. Use terminal(subtool=classic) only for builds, tests, package scripts, and version control; never use terminal for file CRUD or search. Do not read the same AST tree repeatedly or scan nodes sequentially. After a hash conflict, re-read the target and retry with the new hash. Never invent unavailable tools.`;
  ...
  prompt += `\n\nCODE EXPLORATION PROTOCOL: Search first. Use graph/architecture for repo overview, graph/query for dependency hotspots, and graph/impact before risky edits. Prefer search/topographic (AST search) for symbols and paths; use search/classic for content. For trivial work, read/rust with only filePath directly returns file content and hash; read/rust with nodeId must also include the node filePath. Request the AST tree only when hierarchy is needed. Edit by nodeId, unique nodeName, or line range using the returned hash. Edit the smallest stable target; use whole-file AST editing only when most of the file must change.`;
  ...
  if (workspaceTree) {
    ...
    prompt +=
      "\nANTI-LOOP RULE: Read the AST tree once, then request only the target node or range. Do not scan sections sequentially.";
  }
